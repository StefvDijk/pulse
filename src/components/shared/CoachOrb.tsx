...
export interface CoachOrbProps {
  ...
  className?: string
  /**
   * Explicit accent colour (any CSS colour). Tints the disc to the owning
   * coach — e.g. the sport coach's teal. When set it overrides the
   * state-based colour; omit it to keep the manager's state tinting.
   */
  color?: string
}
...
function CoachOrbImpl({ size = 24, state = 'idle', pulsing, className = '', color: colorProp }: CoachOrbProps) {
  const color = colorProp ?? STATE_VAR[state]
