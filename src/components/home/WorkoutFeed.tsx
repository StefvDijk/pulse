    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-text-secondary uppercase tracking-wide">
          Recente trainingen {total > 0 && <span className="text-text-tertiary font-normal">({total})</span>}
        </h2>
        <button
          onClick={handleSync}
          disabled={syncing}
          className="flex items-center gap-1.5 rounded-xl bg-bg-card border border-border-light px-3 py-1.5 text-xs font-medium text-text-secondary hover:border-border-medium disabled:opacity-60 transition-colors"
        >
      ...
      {error && !isLoading && (
        <p className="text-sm text-text-tertiary px-1">Kan trainingen niet laden.</p>
      )}
