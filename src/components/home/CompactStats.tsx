...
const WORKLOAD_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  ...
  optimal: { label: 'Optimaal', color: 'text-system-green', bg: 'bg-system-green/10' },
  high: { label: 'Hoog', color: 'text-amber-400', bg: 'bg-amber-400/10' },
  overreaching: { label: 'Te hoog', color: 'text-system-red', bg: 'bg-system-red/10' },
}
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="rounded-2xl bg-surface-primary border border-separator p-3 text-center">
          ...
          {totalSessions > 0 && (
            <div className="mt-1.5 flex items-center justify-center gap-2 text-[10px] text-label-tertiary">
        ...
        <div className="rounded-2xl bg-surface-primary border border-separator p-3 text-center">
          ...
          <p className="text-xs text-label-tertiary">minuten</p>
        </div>
        ...
        <div className="rounded-2xl bg-surface-primary border border-separator p-3 text-center">
          ...
          <p className="text-xs text-label-tertiary">kg volume</p>
        </div>
      </div>

      {/* ACWR / Workload status */}
      {wl && acwr != null && (
        <div className={`flex items-center justify-between rounded-2xl border border-separator px-4 py-3 ${wl.bg}`}>
          <div className="flex items-center gap-2">
            <Activity size={14} className={wl.color} />
            <span className="text-sm font-medium text-label-primary">Belasting</span>
          </div>
          <div className="flex items-center gap-2">
